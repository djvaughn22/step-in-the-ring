...
// Owner Entrance login shell. Contains no private data. The password goes in
// a POST body over HTTPS to the same server endpoint the Author's Room uses,
// and is never stored anywhere in this browser. After a successful login the
// owner lands on the destination they originally asked for (already validated
// server-side by safeReturnTo — never a raw query value).

import { useState } from "react";
import { useRouter } from "next/navigation";

export default function OwnerLogin({ configured, returnTo }: { configured: boolean; returnTo: string }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setPassword("");
        if (returnTo === "/owner") router.refresh();
  ...
  return (
    <main style={{ maxWidth: 420, margin: "0 auto", padding: "48px 18px" }}>
      <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 16, padding: 22 }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 900, margin: "0 0 8px" }}>Owner Entrance</h1>
        {configured ? (
          <>
            <p style={{ fontSize: 13.5, color: "var(--muted)", lineHeight: 1.55, margin: "0 0 14px" }}>
              The creation rooms are for the site&apos;s owner. Enter the password to continue.
            </p>
            <input
              type="password"
              value={password}
          ...
          <p style={{ fontSize: 13.5, color: "var(--muted)", lineHeight: 1.55, margin: 0 }}>
            The owner entrance isn&apos;t configured on this deployment yet.
          </p>
        )}
